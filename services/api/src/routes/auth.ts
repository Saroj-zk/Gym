...
import {
  ADMIN_COOKIE, MEMBER_COOKIE,
  setAuthCookie, clearAuthCookie, signToken,
  readAuth, requireRole, type AuthedRequest
} from '../utils/auth';
...
router.post('/admin/login', async (req, res) => {
  ...
  if (!password || (!email && !userId)) return res.status(400).json({ error: 'Email/User ID and password required' });
  ...
  res.json({ ok: true, user: { _id: user._id, userId: user.userId, firstName: user.firstName, lastName: user.lastName, email: user.email, role: 'admin' } });
});
...
router.get('/admin/me', readAuth(ADMIN_COOKIE), async (req: AuthedRequest, res) => {
  ...
  res.json({ _id: u._id, userId: u.userId, firstName: u.firstName, lastName: u.lastName, email: u.email, role: 'admin' });
});
...
router.post('/member/login', async (req, res) => {
  ...
  if (!userId && !email) return res.status(400).json({ error: 'User ID or email required' });
  ...
  res.json({ ok: true, user: { _id: user._id, userId: user.userId, firstName: user.firstName, lastName: user.lastName, role: 'member' } });
});
...
router.get('/member/me', readAuth(MEMBER_COOKIE), async (req: AuthedRequest, res) => {
  ...
  res.json({ _id: u._id, userId: u.userId, firstName: u.firstName, lastName: u.lastName, role: 'member' });
});
