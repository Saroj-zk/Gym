...
export default function Payments() {
  ...
  async function savePayment(e: React.FormEvent) {
    ...
    try {
      ...
      await axios.post('/api/payments/record', payload)
      setOpen(false)
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
        ...
        <div className="md:col-span-1">
          ...
          <select value={status} onChange={e=>setStatus(e.target.value)} className="w-full rounded-xl border px-3 py-2">
            <option value="">All</option>
        ...
        <div className="md:col-span-2">
          ...
          <div className="relative">
            ...
            {selectedUser && (
              <button type="button" onClick={()=>{ selectFilterUser(null); setUserQuery(''); }} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500">✕</button>
            )}
          </div>
        </div>
        <div className="md:col-span-6 flex gap-2">
          <button onClick={load} className="px-3 py-2 rounded-xl border">Apply</button>
          <div className="text-sm text-gray-500 self-center">Total: <span className="font-semibold">₹{total}</span></div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed p-10 text-center text-gray-500">No payments found for the selected filters.</div>
      ) : (
            ...
            <tbody>
              {items.map(p => (
                <tr key={p._id} className="border-t">
                  <td className="px-4 py-3">{p.paidAt ? new Date(p.paidAt).toLocaleString() : (p.createdAt ? new Date(p.createdAt).toLocaleString() : '-')}</td>
                  <td className="px-4 py-3">
                    {p?.userId?.userId ? `${p.userId.userId} — ${(p.userId.firstName||'') + ' ' + (p.userId.lastName||'')}` : String(p.userId)}
                  </td>
          ...
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={savePayment} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" value={form.amount} onChange={e=>onChange('amount', e.target.value)} className="w-full rounded-xl border px-3 py-2" placeholder="1200" />
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Method</label>
              ...
              <div className="md:col-span-2">
                ...
                <input value={form.description} onChange={e=>onChange('description', e.target.value)} className="w-full rounded-xl border px-3 py-2" placeholder="Monthly Fee" />
              </div>
              <div className="md:col-span-2 flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 rounded-xl border">Cancel</button>
                <button type="submit" disabled={saving} className="px-4 py-2 rounded-xl bg-black text-white">{saving ? 'Saving…' : 'Save Payment'}</button>
              </div>
            </form>
          </div>
        </div>
