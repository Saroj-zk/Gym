...
export default function Packs() {
  ...
  async function load() {
    try {
      ...
      setError(e?.response?.data?.error || e.message || 'Failed to load packs')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function createPack(e: React.FormEvent) {
    ...
    try {
      ...
      if (!payload.durationValue || payload.durationValue <= 0) throw new Error('Duration value must be > 0')
      ...
      setError(e?.response?.data?.error || e.message || 'Failed to create pack')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  async function toggleActive(pack: Pack) {
    try {
      await axios.patch(`/api/packs/${pack._id}`, { isActive: !pack.isActive })
      await load()
    } catch (e: any) {
      setError(e?.response?.data?.error || e.message || 'Failed to update pack')
    }
  }
            ...
            <tbody>
              {packs.map((p) => (
                <tr key={p._id} className="border-t">
                  ...
                  <td className="px-4 py-3">
                    <span className={"inline-flex items-center px-2 py-1 rounded-full text-xs " + (p.isActive ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-700")}>
          ...
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={createPack} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-1">
                ...
                <input value={form.name} onChange={e=>onChange('name', e.target.value)} className="w-full rounded-xl border px-3 py-2" placeholder="Monthly Standard" />
              </div>
              <div className="col-span-1">
                <label className="block text-sm text-gray-600 mb-1">Price</label>
                <input type="number" value={form.price} onChange={e=>onChange('price', e.target.value)} className="w-full rounded-xl border px-3 py-2" placeholder="1200" />
              </div>
              <div className="col-span-1">
                <label className="block text-sm text-gray-600 mb-1">Duration Type</label>
                <select value={form.durationType} onChange={e=>onChange('durationType', e.target.value as any)} className="w-full rounded-xl border px-3 py-2">
              ...
              <div className="col-span-1">
                ...
                <input type="number" value={form.durationValue} onChange={e=>onChange('durationValue', e.target.value)} className="w-full rounded-xl border px-3 py-2" placeholder="1" />
              </div>
              <div className="col-span-2">
                <label className="block text-sm text-gray-600 mb-1">Description</label>
                <textarea value={form.description} onChange={e=>onChange('description', e.target.value)} className="w-full rounded-xl border px-3 py-2" rows={3} placeholder="Access to all equipment" />
              </div>
              <div className="col-span-2">
                <label className="block text-sm text-gray-600 mb-1">Features (comma-separated)</label>
                <input value={form.featuresCsv} onChange={e=>onChange('featuresCsv', e.target.value)} className="w-full rounded-xl border px-3 py-2" placeholder="cardio, weights, sauna" />
              </div>
              <div className="col-span-2 flex items-center gap-2">
                <input id="active" type="checkbox" checked={form.isActive} onChange={e=>onChange('isActive', e.target.checked)} />
